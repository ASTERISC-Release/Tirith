...
void glXSwapBuffers(Display* dpy, GLXDrawable drawable)
{
    ...
    /* Adil */
    // fprintf(stderr, "glXSwapBuffers called\n");
    ...
    /* Adil: If using host SDL window, skip XPutImage */
    if (!host_sdl_window) {
      /* display */
    ...
    // fprintf(stderr, "before XSync..\n");

    /* Adil: If using host SDL window, do a sync to ensure rendering is done */ 
    if (!host_sdl_window) {
      /* sync */
      XSync(dpy, False);
    }

    // fprintf(stderr, "glXSwapBuffers completed\n");
