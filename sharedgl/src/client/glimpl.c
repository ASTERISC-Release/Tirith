...
// adil 
extern bool host_sdl_window;

static inline void init_shm(bool use_direct_access)
{
#ifndef _WIN32
    /* Adil: check if environment variable is set  */ 
    printf("init_shm: using shared memory (%s)\n", SGL_SHARED_MEMORY_NAME);
    char *ivshmem_gramine_vm = getenv("IVSHMEM_GRAMINE_VM");

    /* Adil: In Gramine VM, the shared memory region is pre-mapped. */
    if (ivshmem_gramine_vm == NULL) {
      printf("init_shm: assuming non-gramine vm\n");
      ...
      printf("init_shm: assuming gramine vm\n");
      int fd = 0; // TODO: detect dynamically

      printf("Setting for direct usage\n");
      pb_set(fd, true); // Adil: force direct usage
      printf("Initialized\n");
    } 
...
static inline void shm_create_context()
{
    ...
    //fprintf(stderr, "shm_create_context: waiting for server to accept\n");

    /*
     * submit
     */
    pb_push(SGL_CMD_CREATE_CONTEXT);
    // adil: inform server if using host sdl window
    if (host_sdl_window)
        pb_push(1);
    else
        pb_push(0);
    
    //fprintf(stderr, "shm_create_context: submitting create context command\n");

    glimpl_submit();

    //fprintf(stderr, "shm_create_context: server accepted, reading max dimensions\n");
...
static inline void init_net(char *network)
{
    ...
    fprintf(stderr, "Connected to the Host\n");
...
void glimpl_init()
{
    ...
    /* Adil */
    fprintf(stderr, "Obtained SharedGL context.\n");
