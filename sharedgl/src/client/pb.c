...
void pb_set(int fd, bool direct_access)
{
    ...
    char *ivshmem_gramine_vm = getenv("IVSHMEM_GRAMINE_VM"); // Adil: Gramine sets this env var

    /* Adil: If IVSHMEM_GRAMINE_VM is defined, we simply pick the default range for now */
    if (ivshmem_gramine_vm == NULL) {
      printf("Not running in Gramine VM, mapping shared memory region\n");
      ...
      printf("Detected Gramine VM, using default ivshmem range\n");
      ptr = (void*) 0xc4000000; // TODO: detect dynamically
    }

    base = ptr + 0x1000;

    if (direct_access) {
        fprintf(stderr, "Using direct access to ivshmem region\n");
        using_direct_access = true;
        in_base = base;
        in_cur = base;
        fprintf(stderr, "in_base: %p\n", in_base);
    }
    else {
        printf("Using indirect access to ivshmem region\n");
