...
/* Adil: Enable VSOCK by default to support QEMU/KVM */
bool VSOCK_ENABLE = false;
extern void* x11_main(void* ptr);
#include <pthread.h>
...
static const char *usage =
    ...
    "    -s          if networking is enabled, leverage vscock from guest\n";
...
int main(int argc, char **argv)
{
  ...
  #if 0 
    /* Adil: start background X11 display handler */
    pthread_t sgl_x11_thread;
    if (pthread_create(&sgl_x11_thread, NULL, (void*(*)(void*))x11_main, NULL) != 0) {
        PRINT_LOG("failed to start X11 handler\n");
        return -3;
    }
  #endif
