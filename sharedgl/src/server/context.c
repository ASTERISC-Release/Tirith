...
/* Adil: SDL-Window at Host, and direct display */
extern bool host_sdl_window;
static int frame_buffer_call_first_time = -1;
...
struct sgl_host_context *sgl_context_create()
{
    ...
    /* Adil: Create a direct on-screen window */
    if (host_sdl_window) {
    fprintf(stderr, "Creating SDL ON SCREEN Window of size %dx%d\n", mw, mh);
    context->window = SDL_CreateWindow(
      ...
      fprintf(stderr, "Creating SDL OFF SCREEN Window of size %dx%d\n", mw, mh);
      context->window = SDL_CreateWindow(
    ...
    /* Adil: Enable V-Sync for some reason. (TODO: check later) */
    if (host_sdl_window) {
      int ret = SDL_GL_SetSwapInterval(0) ;
      SDL_SetHint(SDL_HINT_RENDER_VSYNC, "1");
      if(ret)
          fprintf(stderr, "Error: SDL_GL_SetSwapInterval returned -1.\n");
    }
...
/* Adil: In case of HOST_SDL_WINDOW, we directly display the frames. */
void *sgl_read_pixels_host(unsigned int width, unsigned int height, void *data, 
                            int vflip, int format, size_t mem_usage, 
                            struct sgl_host_context *ctx)
{
    if (frame_buffer_call_first_time != ctx->client_id){
        sgl_set_max_resolution(width, height);
        frame_buffer_call_first_time = ctx->client_id;
    }
