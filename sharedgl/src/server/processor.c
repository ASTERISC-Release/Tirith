...
void sgl_cmd_processor_start(struct sgl_cmd_processor_args args)
{
    ...
    *(int*)(p + SGL_OFFSET_REGISTER_SWAP_BUFFERS_SYNC) = 0;
    ...
    // Adil: keeping count of received commands
    ...
    while (1) {
        ...
        while (*pb != SGL_CMD_INVALID) {
            // Adil: incrementing count and printing
            command_count++;
            ...
            case SGL_CMD_CREATE_CONTEXT:
                // PRINT_LOG("SGL_CMD_CREATE_CONTEXT command received ... \n");
                *(int*)(p + SGL_OFFSET_REGISTER_RETVAL) = PACK(width, height);
                host_sdl_window = *pb++;  // adil: check if host sdl window is required
                if (host_sdl_window) {
                  PRINT_LOG("Creating context with HOST SDL WINDOW\n");
                  /* Find the connection for this client_id */
                  struct sgl_connection *con;
                  ...
                  PRINT_LOG("Using context with GUEST SDL WINDOW\n");
                } 
                break;
            ...
            case SGL_CMD_SWAP_BUFFERS: {
                ...
                if (host_sdl_window) {
                  ...
                  for (con = connections; con; con = con->next)
                      if (con->id == client_id)
                          break;
                  con->ctx->client_id = client_id;

                  /* Read pixels and directly display on host SDL window */
                  sgl_read_pixels_host(w, h, p + SGL_OFFSET_COMMAND_START + fifo_size, vflip, 
                                      format, (size_t)pb - (size_t)(p + SGL_OFFSET_COMMAND_START), 
                                       (struct sgl_host_context*)con->ctx);
                } else {
                  sgl_read_pixels(w, h, p + SGL_OFFSET_COMMAND_START + fifo_size, vflip, 
                                    format, (size_t)pb - (size_t)(p + SGL_OFFSET_COMMAND_START));
                } 
