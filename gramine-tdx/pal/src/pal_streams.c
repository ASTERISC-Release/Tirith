...
int _PalStreamOpen(PAL_HANDLE* handle, const char* typed_uri, enum pal_access access,
    ...
    // Adil 
    // log_always("PalStreamOpen: type=%s, uri=%s, access=0x%x, share=0x%x, create=%d, options=0x%x",
...
int _PalStreamMap(PAL_HANDLE handle, void* addr, pal_prot_flags_t prot, uint64_t offset,
    ...
    // log_always("[mmap] file mapping request: addr=%p, length=%zu, prot=0x%x, offset=%lu", addr, size,
    //            prot, offset);
