...
static int proc_maps_info_callback(struct proc_maps_range* r, void* arg) {
    ...
    return pal_add_initial_range(r->start, r->end - r->start, r->prot,
                                 /*is_user_reserved=*/false, r->name ?: "");
}
...
#error "Your arch grows stack towards high addresses, this is not supported."
    ...
    assert(mmap_ret == ptr);
    ret = pal_add_initial_range((uintptr_t)ptr, PAGE_SIZE, /*prot=*/0,
                                /*is_user_reserved=*/false, "stack guard");
    if (ret < 0) {
        return ret;
    }
