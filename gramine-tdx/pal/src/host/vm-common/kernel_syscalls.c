...
#ifdef AMD_CPU_FIX
    /* STAR wants user CS selector minus 16, with RPL=3 set (sysretq will load CS=+16, SS=+8) */
    uint64_t user_cs_sel = (uint64_t)(gdt_entry_user_cs - gdt_start) | 0x3;
    wrmsr(MSR_STAR,
        (user_cs_sel << 48) | ((gdt_entry_kernel_cs - gdt_start) << 32));
#else 
    wrmsr(MSR_STAR,
