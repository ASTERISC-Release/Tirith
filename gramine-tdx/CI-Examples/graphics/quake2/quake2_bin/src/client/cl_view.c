...
void V_TestParticles(void) {
    ...
    for (i = 0; i < r_numparticles; i++) {
        ...
        for (j = 0; j < 3; j++) {
            p->origin[j] =
                cl.refdef.vieworg[j] + cl.v_forward[j] * d + cl.v_right[j] * r + cl.v_up[j] * u;
        }
...
void CL_PrepRefresh(void) {
    ...
    Com_Printf("STARTING: %s\r", mapname);
    SCR_UpdateScreen();
    Com_Printf("Update complete: %s\r", mapname);
    R_BeginRegistration(mapname);
    Com_Printf("Registration complete: %s\r", mapname);
    Com_Printf("                                     \r");
    ...
    for (i = 1; i < MAX_MODELS && cl.configstrings[CS_MODELS + i][0]; i++) {
        ...
        if (name[0] == '#') {
            ...
            if (num_cl_weaponmodels < MAX_CLIENTWEAPONMODELS) {
                Q_strlcpy(cl_weaponmodels[num_cl_weaponmodels], cl.configstrings[CS_MODELS + i] + 1,
...
int entitycmpfnc(const entity_t* a, const entity_t* b) {
    ...
    if (a->model == b->model) {
        return (a->skin == b->skin) ? 0 : (a->skin > b->skin) ? 1 : -1;
    } else {
...
void V_Render3dCrosshair(void) {
    ...
    if (crosshair_3d->value || crosshair_3d_glow->value) {
        ...
        if (crosshair_3d_glow->value) {
            ...
            V_AddLight(crosshair_trace.endpos, crosshair_3d_glow->value, crosshair_3d_glow_r->value,
                       crosshair_3d_glow_g->value, crosshair_3d_glow_b->value);
        }
