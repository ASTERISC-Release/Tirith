...
static int IN_TranslateScancodeToQ2Key(SDL_Scancode sc) {
#define MY_SC_CASE(X)      \
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
                ...
                if (sc >= SDL_SCANCODE_1 && sc <= SDL_SCANCODE_0) {
                    ...
                    if (sc == SDL_SCANCODE_GRAVE && kc != '\'' && kc != '"') {
                        ...
                        if ((event.key.keysym.mod &
                             (KMOD_CAPS | KMOD_SHIFT | KMOD_ALT | KMOD_CTRL | KMOD_GUI)) == 0) {
                            // also, only do this if no modifiers like shift or AltGr or whatever
                            // are pressed so kc will most likely be the ascii char generated by
                            // this and can be ignored in case SDL_TEXTINPUT above (so we don't get
                            // ^ or whatever as text in console) (can't just check for mod == 0
                            // because numlock is a KMOD too)
                        ...
                        if (key != 0) {
                            ...
                            Com_DPrintf(
                                "Pressed unknown key with SDL_Keycode %d, SDL_Scancode %d.\n", kc,
                                (int)sc);
                        }
                    }
                }
            ...
            case SDL_WINDOWEVENT:
                if (event.window.event == SDL_WINDOWEVENT_FOCUS_LOST ||
                    ...
                    if (event.window.event == SDL_WINDOWEVENT_FOCUS_LOST) {
                        ...
                        /* pause music */
                        if (Cvar_VariableValue("ogg_pausewithgame") == 1 && OGG_Status() == PLAY &&
                    ...
                    if (event.window.event == SDL_WINDOWEVENT_FOCUS_GAINED) {
                        ...
                        /* play music */
                        if (Cvar_VariableValue("ogg_pausewithgame") == 1 && OGG_Status() == PAUSE &&
                            cl.attractloop == false && cl_paused->value == 0) {
                            Cbuf_AddText("ogg toggle\n");
                        }
                    }
            ...
            case SDL_CONTROLLERBUTTONDOWN: {
                ...
                Key_Event((btn == sdl_back_button) ? K_JOY_BACK : K_BTN_A + btn, down, true);
                break;
            }
            ...
            case SDL_JOYAXISMOTION:
                ...
                if (gyro_active && gyro_mode->value && !cl_paused->value &&
            ...
            case SDL_JOYDEVICEADDED:
                if (!controller) {
                    // This should be lower, but some controllers just don't want to get detected by
                    // the OS
            ...
            case SDL_JOYBATTERYUPDATED:
                ...
                if (event.jbattery.level == SDL_JOYSTICK_POWER_LOW) {
                    Com_Printf(
                        "WARNING: Gamepad battery Low, it is recommended to connect it by "
                        "cable.\n");
            ...
            switch (countdown_reason) {
                ...
                case REASON_GYROCALIBRATION:  // finish and save calibration
                {
                    ...
                    if (!num_samples[0] || !num_samples[1] || !num_samples[2]) {
                        Com_Printf(
                            "Calibration failed, please retry inside a level after having moved "
                            "your controller a little.\n");
                    } else {
 ...
 */
static float IN_StickMagnitude(thumbstick_t stick) {
 ...
 */
static float IN_MapRange(float v, float deadzone, float sign) {
 ...
 */
static thumbstick_t IN_RadialDeadzone(thumbstick_t stick, float deadzone) {
 ...
 */
static thumbstick_t IN_SlopedAxialDeadzone(thumbstick_t stick, float deadzone) {
    thumbstick_t result = {0};
 ...
 */
static thumbstick_t IN_ApplyExpo(thumbstick_t stick, float exponent) {
    thumbstick_t result = {0};
 ...
 */
static thumbstick_t IN_TightenInput(float yaw, float pitch) {
 ...
 */
static void IN_ResetSmoothSamples() {
    front_sample = 0;
 ...
 */
static float IN_SmoothedStickRotation(float value) {
    ...
    float immediate_weight = (fabsf(value) - bottom_threshold) / (top_threshold - bottom_threshold);
 ...
 */
static float IN_FlickStick(thumbstick_t stick, float axial_deadzone) {
...
void IN_Move(usercmd_t* cmd) {
    ...
    static const float rotation_factor[FLICK_TIME] = {0.305555556f, 0.249999999f, 0.194444445f,
                                                      0.138888889f, 0.083333333f, 0.027777778f};
    ...
    if (mouse_x || mouse_y) {
        if (!exponential_speedup->value) {
            ...
            if ((mouse_x > MOUSE_MIN) || (mouse_y > MOUSE_MIN) || (mouse_x < -MOUSE_MIN) ||
                (mouse_y < -MOUSE_MIN)) {
    ...
    if (joystick_yaw) {
        cl.viewangles[YAW] -=
            (m_yaw->value * joy_yawsensitivity->value * cl_yawspeed->value * joystick_yaw) *
            joyViewFactor;
    }

    if (joystick_pitch) {
        cl.viewangles[PITCH] +=
            (m_pitch->value * joy_pitchsensitivity->value * cl_pitchspeed->value * joystick_pitch) *
            joyViewFactor;
    }
    ...
    if (joystick_sidemove) {
        // We need to be twice as fast because with joystick we run...
        cmd->sidemove += m_side->value * joy_sidesensitivity->value * cl_sidespeed->value * 2.0f *
                         joystick_sidemove;
    }
    ...
    if (gyro_in.x) {
        cl.viewangles[YAW] += m_yaw->value * gyro_yawsensitivity->value * cl_yawspeed->value *
                              gyro_in.x * gyroViewFactor;
    }
...
static void IN_JoyAltSelectorDown(void) {
    joy_altselector_pressed = true;
}

static void IN_JoyAltSelectorUp(void) {
 ...
 * Init haptic effects
 */
static int IN_Haptic_Effect_Init(int effect_x, int effect_y, int effect_z, int period,
                                 int magnitude, int delay, int attack, int fade) {
...
static int IN_Haptic_GetEffectId(int effect_volume, int effect_duration, int effect_delay,
                                 int effect_attack, int effect_fade, int effect_x, int effect_y,
                                 int effect_z) {
    int i, haptic_volume;

    // check effects for reuse
    for (i = 0; i < last_haptic_effect_size; i++) {
        if (last_haptic_effect[i].effect_volume != effect_volume ||
    ...
    */
    ...
    last_haptic_effect[last_haptic_effect_pos].effect_id =
        IN_Haptic_Effect_Init(effect_x, effect_y, effect_z, effect_duration, haptic_volume,
                              effect_delay, effect_attack, effect_fade);
 ...
 */
static qboolean Haptic_Feedback_Filtered_Line(const char* name, const char* filter) {
 ...
 */
static qboolean Haptic_Feedback_Filtered(const char* name, const char* filter) {
    const char* current_filter = filter;
 ...
 */
void Haptic_Feedback(const char* name, int effect_volume, int effect_duration, int effect_delay,
                     int effect_attack, int effect_fade, int effect_x, int effect_y, int effect_z,
    ...
    if (!joystick_haptic || joy_haptic_magnitude->value <= 0 ||
        ...
        effect_distance > max_distance || effect_volume <= 0 || effect_duration <= 0 ||
    ...
    if (Haptic_Feedback_Filtered(name, haptic_feedback_filter->string)) {
        ...
        effect_id = IN_Haptic_GetEffectId(effect_volume, effect_duration, effect_delay,
                                          effect_attack, effect_fade, effect_x, effect_y, effect_z);
 ...
 */
void Controller_Rumble(const char* name, vec3_t source, qboolean from_player, unsigned int duration,
    ...
    if (!show_haptic || !controller || joy_haptic_magnitude->value <= 0 || volume == 0 ||
        duration == 0) {
        return;
    }
...
static void IN_Controller_Init(qboolean notify_user) {
...
#ifdef SDL_HINT_JOYSTICK_HIDAPI_PS4_RUMBLE  // extended input reports on PS controllers (enables
                                            // gyro thru bluetooth)
    ...
    if (!SDL_NumJoysticks()) {
        ...
        if (joystick_haptic && (SDL_HapticQuery(joystick_haptic) & SDL_HAPTIC_SINE) == 0) {
    ...
    for (const char* rawPath = FS_GetNextRawPath(NULL); rawPath != NULL;
    ...
    for (int i = 0; i < SDL_NumJoysticks(); i++) {
        ...
        if (name_len > 4 && !strncmp(joystick_name + name_len - 4, " IMU", 4)) {
            ...
            if (!imu_joystick && name_len > 16 &&
                strncmp(joystick_name + name_len - 16, "Left Joy-Con IMU", 16) != 0) {
        ...
        if (!is_controller) {
            ...
            Com_Printf(
                "To use joystick as game controller, provide its config by either:\n"
