...
static int setup_syscall_comms(void) {
    fprintf(stderr, "\n========COMMS========\n");
    ...
    *(volatile uint64_t*)addr = 0x1234567812345678ULL;
    __sync_synchronize();
    sleep(2); // Ensure that sg-listener is truly awake.
    fprintf(stderr, "[*] COMMS: 0x%lx\n", SYS_COMMS_ADDR);

    /* Setup the data region. */
    volatile void* data = mmap(DATA_REGION, DATA_SIZE, 
    ...
    *(volatile uint64_t*)data = 0x1234567812344678ULL;
    __sync_synchronize();
    ...
    volatile void* hugepage_data = mmap(HUGEPAGE_DATA_REGION, DATA_SIZE, 
                      PROT_READ | PROT_WRITE,
                      MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
    ...
    memset((void*)hugepage_data, 0, DATA_SIZE);
...
static void setup_gbm_device(void) {
    ...
    int drm_fd = open(drm_node, O_RDWR | O_CLOEXEC | O_NONBLOCK);  // sent ot the host
    if (drm_fd < 0) {
        perror("open(drm)");
        assert(0);
        return;
    ...
    fprintf(stderr, "[*] Device (gbm) creation success\n");
...
void __attribute__((constructor)) sharedgl_entry(void) {
    /* Setup communication regions */
    setup_syscall_comms(); 
    
    /* Enable logging */
    // enable_syscall_logging();
...
Bool glXMakeCurrent(Display* dpy, GLXDrawable drawable, GLXContext ctx1) {
    ...
    /* Disable syscall logging */
    disable_syscall_logging();
