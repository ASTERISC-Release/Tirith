...
int setup_egl()
{
    ...
    if (!eglChooseConfig(eglDpy, cfgAttrs, &cfg, 1, &numCfg) || numCfg == 0) {
        fprintf(stderr,"[X] eglChooseConfig failed\n");
        assert(0);
        return 1;
    }
    fprintf(stderr, "[.] EGL choose config success\n");
    ...
    fprintf(stderr,"[*] EGL Bound API success (%p)\n", eglCreateContext);
    ...
    fprintf(stderr, "[*] EGL context created\n");
    ...
    glClientWaitSync_ptr = (PFNGLCLIENTWAITSYNCPROC)eglGetProcAddress("glClientWaitSync");
    glDeleteSync_ptr = (PFNGLDELETESYNCPROC)eglGetProcAddress("glDeleteSync");
    glFramebufferRenderbuffer_ptr = (PFNGLFRAMEBUFFERRENDERBUFFERPROC)eglGetProcAddress(
    ...
    if (!eglCreateImageKHR_ptr || !eglDestroyImageKHR_ptr || !glEGLImageTargetTexture2DOES_ptr ||
        ...
        fprintf(stderr,"[X] Missing required EGL/GL entry points\n");
        assert(0); 
        return 1;
    }
    ...
    for (int i = 0; i < NUM_BUFFERS; ++i) {
        ...
        */
        ...
        fprintf("[.] dup() file descriptor for bo_fd=%d of buffer %d\n", bufs[i].bo_fd, i);
        int test_fd = dup(bufs[i].bo_fd); //to be returned.


        // test_fd is the integer you read from shared memory
        int flags = fcntl(test_fd, F_GETFD);
        if (flags == -1) {
            fprintf(stderr, "warning: test_fd (%d) invalid in this process\n", test_fd);
        } else {
        ...
        /*
            This is the idea:
                ...
                it maily calls this ioctl: DRM_IOCTL_PRIME_FD_TO_HANDLE
